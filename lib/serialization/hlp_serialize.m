...
function m = serialize_object(v)
    try
        ...
        if isstruct(conts) || iscell(conts) || isnumeric(conts) || ischar(conts) || islogical(conts) || isa(conts,'function_handle')
            ...
            warning('\nhlp_serialize: serializing object of class ''%s'' (saveobj() returned ''%s'')\n',class(v),class(conts));
        ...
        fprintf('Serialization of object saveobj(''%s'') failed, turning it into a structure\n',class(v));
